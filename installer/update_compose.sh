...
# from the deploy-config-${ENV_NAME} branch and restarts the Refinery
...
WORKING_DIRECTORY="${2:-$HOME/refinery-validator/}"
...
COMPOSE_TEMP="$(mktemp "${TMPDIR:-/tmp}/refinery_update.XXXXXX")"
TEMP_FILES+=("${COMPOSE_TEMP}")
ALLOY_TEMP="$(mktemp "${TMPDIR:-/tmp}/refinery_update.XXXXXX")"
