#!/usr/bin/env bash
# Installer for the Refinery validator.
...
if [ ! -f "${ENV_FILE}" ]; then
    ...
    read -r -p "Enter BITTENSOR_NETWORK [local]: " BITTENSOR_NETWORK </dev/tty
    BITTENSOR_NETWORK=${BITTENSOR_NETWORK:-local}
...
CRON_CMD="*/15 * * * * curl -fsSL ${UPDATE_URL_Q} -o ${UPDATE_SCRIPT_Q} && chmod +x ${UPDATE_SCRIPT_Q} && ${UPDATE_SCRIPT_Q} ${ENV_NAME_Q} ${WORKING_DIRECTORY_Q} # REFINERY_VALIDATOR_UPDATE"

EXISTING_CRONTAB="$(crontab -l 2>/dev/null || true)"
FILTERED_CRONTAB="$(printf "%s\n" "${EXISTING_CRONTAB}" | grep -F -v "REFINERY_VALIDATOR_UPDATE" || true)"
