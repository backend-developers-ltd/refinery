...
**Refinery** is a deliberately minimal, **local-only** Bittensor subnet whose only purpose is to generate
genuine, verifiable validator↔miner traffic on a local subtensor, so we can exercise and test the tooling
we build around subnets (Pylon, the Nexus validator runtime, observability, weight setting, deployment,
logging). It is never deployed to testnet or mainnet and has no economic value.

The task is a trivial proof-of-work: each block the validator challenges a miner to find a `nonce` such
that `SHA-256(seed || nonce)` has `difficulty` leading zero bits; it verifies with one hash and, each
epoch, weighs miners by correctness and speed. Full design: `subnet_design.md`. Validator code lives in
`validator/` (Nexus); `miner/` holds a simple brute-force **test-fixture** miner with a configurable
response delay. `difficulty` is the load knob.

> Origin: rendered from the Nexus subnet template (`bittensor-church/nexus-subnet-template`).
